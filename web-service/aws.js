...
const gc_config = ini.parse(fs.readFileSync(config.gc_config, 'utf-8'));
const s3_region_name = gc_config.s3readonly.region_name;
const s3_bucket_name = gc_config.s3.bucket_name;
...
let s3 = new S3Client({
  ...
  credentials: fromIni({ profile: config.gc_profile })
});
